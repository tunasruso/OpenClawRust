...
use api::{
    openai_chatgpt_oauth_config_from_env, resolve_startup_auth_source, AuthSource, ClawApiClient,
    ContentBlockDelta, InputContentBlock, InputMessage, MessageRequest, MessageResponse,
    OutputContentBlock, OpenAiCompatClient, OpenAiCompatConfig, ProviderClient,
    OPENAI_CHATGPT_CREDENTIALS_KEY,
...
use runtime::{
    clear_oauth_credentials, clear_provider_oauth_credentials, generate_pkce_pair,
    generate_state, load_system_prompt, parse_oauth_callback_request_target,
    save_oauth_credentials, save_provider_oauth_credentials, ApiClient, ApiRequest,
...
impl LoginProvider {
    fn parse(value: &str) -> Result<Self, String> {
        match value.trim().to_ascii_lowercase().as_str() {
            "" | "claw" | "anthropic" => Ok(Self::Claw),
            "openai-chatgpt" | "openai" | "chatgpt" | "codex" => Ok(Self::OpenAiChatGpt),
            other => Err(format!(
                "unsupported login provider: {other} (expected claw or openai-chatgpt)"
            )),
        }
    }
}
...
fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    match rest[0].as_str() {
        ...
        "login" => Ok(CliAction::Login {
            provider: parse_login_provider_arg(&rest[1..])?,
        }),
...
fn parse_login_provider_arg(args: &[String]) -> Result<LoginProvider, String> {
    match args {
        [] => Ok(LoginProvider::Claw),
        [provider] => LoginProvider::parse(provider),
        _ => Err("login/logout accept at most one optional provider argument".to_string()),
    }
}
...
fn run_login(provider: LoginProvider) -> Result<(), Box<dyn std::error::Error>> {
    match provider {
        LoginProvider::Claw => run_claw_login(),
        LoginProvider::OpenAiChatGpt => run_openai_chatgpt_login(),
    }
}

fn run_claw_login() -> Result<(), Box<dyn std::error::Error>> {
...
fn run_openai_chatgpt_login() -> Result<(), Box<dyn std::error::Error>> {
    let oauth = openai_chatgpt_oauth_config_from_env()?.ok_or_else(|| {
        io::Error::other(
            "OpenAI ChatGPT login requires OPENAI_CHATGPT_CLIENT_ID, OPENAI_CHATGPT_AUTHORIZE_URL, and OPENAI_CHATGPT_TOKEN_URL",
        )
    })?;
    let callback_port = oauth.callback_port.unwrap_or(DEFAULT_OAUTH_CALLBACK_PORT);
    let redirect_uri = runtime::loopback_redirect_uri(callback_port);
    ...
    println!("Starting OpenAI ChatGPT OAuth login...");
    println!("Listening for callback on {redirect_uri}");
    if let Err(error) = open_browser(&authorize_url) {
        eprintln!("warning: failed to open browser automatically: {error}");
        println!("Open this URL manually:\n{authorize_url}");
    ...
    let exchange_request =
        OAuthTokenExchangeRequest::from_config(&oauth, code, state, pkce.verifier, redirect_uri);
    let runtime = tokio::runtime::Runtime::new()?;
    let token_set = runtime.block_on(
        OpenAiCompatClient::new(String::new(), OpenAiCompatConfig::openai())
            .exchange_oauth_code(&oauth, &exchange_request),
    )?;
    save_provider_oauth_credentials(OPENAI_CHATGPT_CREDENTIALS_KEY, &token_set)?;
    println!("OpenAI ChatGPT OAuth login complete.");
    Ok(())
}

fn run_logout(provider: LoginProvider) -> Result<(), Box<dyn std::error::Error>> {
    match provider {
        LoginProvider::Claw => {
            clear_oauth_credentials()?;
            println!("Claw OAuth credentials cleared.");
        }
        LoginProvider::OpenAiChatGpt => {
            clear_provider_oauth_credentials(OPENAI_CHATGPT_CREDENTIALS_KEY)?;
            println!("OpenAI ChatGPT OAuth credentials cleared.");
        }
    }
    Ok(())
}
...
impl DefaultRuntimeClient {
    fn new(
        ...
        Ok(Self {
            ...
            client: resolve_runtime_provider_client(&model)?,
            model,
            enable_tools,
            emit_output,
            allowed_tools,
...
fn resolve_runtime_provider_client(model: &str) -> Result<ProviderClient, Box<dyn std::error::Error>> {
    Ok(match api::detect_provider_kind(model) {
        api::ProviderKind::ClawApi => ProviderClient::ClawApi(
            ClawApiClient::from_auth(resolve_startup_auth_source(|| {
...
fn print_help_to(out: &mut impl Write) -> io::Result<()> {
    ...
    writeln!(
        out,
        "  claw login [claw|openai-chatgpt]     Start the OAuth login flow"
    )?;
    writeln!(
        out,
        "  claw logout [claw|openai-chatgpt]    Clear saved OAuth credentials"
    )?;
    writeln!(
        out,
    ...
    writeln!(out, "  claw login openai-chatgpt")?;
...
mod tests {
    ...
    fn parses_login_and_logout_subcommands() {
        ...
        assert_eq!(
            parse_args(&["login".to_string(), "openai-chatgpt".to_string()])
                .expect("openai login should parse"),
            CliAction::Login {
                provider: LoginProvider::OpenAiChatGpt,
            }
        );
        assert_eq!(
