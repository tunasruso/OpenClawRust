...
const DEFAULT_OAUTH_CREDENTIALS_KEY: &str = "oauth";
...
pub fn load_oauth_credentials() -> io::Result<Option<OAuthTokenSet>> {
    load_provider_oauth_credentials(DEFAULT_OAUTH_CREDENTIALS_KEY)
}

pub fn load_provider_oauth_credentials(provider_key: &str) -> io::Result<Option<OAuthTokenSet>> {
    let path = credentials_path()?;
    let root = read_credentials_root(&path)?;
    let Some(oauth) = root.get(provider_key) else {
        return Ok(None);
    };
    if oauth.is_null() {
        return Ok(None);
...
pub fn save_oauth_credentials(token_set: &OAuthTokenSet) -> io::Result<()> {
    save_provider_oauth_credentials(DEFAULT_OAUTH_CREDENTIALS_KEY, token_set)
}

pub fn save_provider_oauth_credentials(
    provider_key: &str,
...
pub fn clear_oauth_credentials() -> io::Result<()> {
    clear_provider_oauth_credentials(DEFAULT_OAUTH_CREDENTIALS_KEY)
}

pub fn clear_provider_oauth_credentials(provider_key: &str) -> io::Result<()> {
    let path = credentials_path()?;
    let mut root = read_credentials_root(&path)?;
...
mod tests {
    ...
    use super::{
        clear_oauth_credentials, clear_provider_oauth_credentials, code_challenge_s256,
        credentials_path, generate_pkce_pair, generate_state, load_oauth_credentials,
        load_provider_oauth_credentials, loopback_redirect_uri, parse_oauth_callback_query,
        parse_oauth_callback_request_target, save_oauth_credentials,
        save_provider_oauth_credentials, OAuthAuthorizationRequest, OAuthConfig,
    ...
    #[test]
    fn provider_oauth_credentials_round_trip_without_touching_default_slot() {
        let _guard = env_lock();
        let config_home = temp_config_home();
        std::env::set_var("CLAW_CONFIG_HOME", &config_home);
        ...
        save_provider_oauth_credentials("openaiChatgptOauth", &token_set)
            .expect("save provider credentials");
        assert_eq!(load_oauth_credentials().expect("load default"), None);
        assert_eq!(
            load_provider_oauth_credentials("openaiChatgptOauth").expect("load provider"),
            Some(token_set)
        );

        clear_provider_oauth_credentials("openaiChatgptOauth")
            .expect("clear provider credentials");
        assert_eq!(
            load_provider_oauth_credentials("openaiChatgptOauth").expect("load cleared provider"),
            None
        );
